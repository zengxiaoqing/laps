...
!	dir_s = '../static/' 
	...
	ext_s = laps_domain
c
        call get_directory('static',dir_s,len_dir)

	var_s = 'LAT'
	call rd_laps_static(dir_s,ext_s,ni,nj,3,var_s,units,comment,
...
c
	...
	enddo !j
c
        call get_directory('static',fname,len_fname)        

	open(19,file=fname(1:len_fname)//'drag_coef.dat',
...
c
	...
	do i=1,ni
	  if(topo(i,j) .gt. termx) termx = topo(i,j)
        ...
        enddo !j
c
c        kdim = 5   !0.01 radius of infl.
        npass = 1
	smsng = 1.e37
c
	rom2 = 0.01
	call dynamic_wts(imax,jmax,n_obs_var,rom2,d)
	call barnes2(top,imax,jmax,topo,smsng,kdim,npass,d1,d2)
c
	topmx = 0.
	do j=1,nj
	do i=1,ni
...
c
	open(10,file=fname(1:len_fname)//'pbl_top.dat',
     &          form='unformatted',status='unknown')
	write(10) top
	close(10)
	print *,' '
