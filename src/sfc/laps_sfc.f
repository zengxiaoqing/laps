...
!       Declarations for this routine's 'read_sfc_snd' argument list
	real lat_s(mxstn), lon_s(mxstn), elev_s(mxstn)
	real t_s(mxstn), t_ea(mxstn), max24t(mxstn), min24t(mxstn)
        real td_s(mxstn), td_ea(mxstn), rh_s(mxstn), rh_ea(mxstn)
...
!       Append sounding obs to surface arrays
        do i_pr = 1,n_profiles
            ...
            if(nlevels_obs_pr(i_pr) .gt. 0)then
                ...
                rlevel_best_wind      = 99999.
                ilevel_best_wind      = 0

                rlevel_best_temp_diff = 99999.
                rlevel_best_temp      = 99999.
                ilevel_best_temp      = 0
...
!                       rlevel_best_temp      = ob_pr_ht_obs(i_pr,il)
     ...
     1                       ,rlevel_best_wind_diff
     1                       ,rlevel_best_temp_diff,c5_name(i_pr)       
                    l_good_snd = .true.
...
!               Add wind ob into arrays
     ...
     1                             ,ob_pr_u_obs(i_pr,ilevel_best_wind)
     1                             ,dd_s(n_obs_b)
     1                             ,speed_ms )
...
!               Add temperature ob into arrays (if returned from 
!                                               'read_snd_data' )
                write(6,*)' Temp obs not yet coded'
