...
#define create_bcd_bkgnd_xy create_bcd_bkgnd_xy_
#endif
#ifdef FORTRANDOUBLEUNDERSCORE
#define create_bcd_bkgnd create_bcd_bkgnd__
#define create_bcd_bkgnd_xy create_bcd_bkgnd_xy__
#endif
...
void dump_xy_to_gnuplot(FILE * fp, float *x, float *y, int npts, float cornertocorner)
{
    int i;
...
void create_bcd_bkgnd_xy(int * proj_idx,
            float * cenlat,
            float * cenlon,
            float * rot,
            float * x1lat1,
...

{
  ...
  ofp1 = fopen("centerpoint","w");
  if (ofp1==NULL) {
    fprintf(stderr,"Could not open centerpoint for writing\n");
    exit(1);
  }/*endif*/
  fprintf(ofp1,"0.0 0.0\n");
  fclose (ofp1);
  ...
  while (1) {
    ...
    for (ii=0; ii<oneRecord.npts; ii++) {
      ll_to_xy(&(oneRecord.pts[ii].lat),&(oneRecord.pts[ii].lon),
	       x+ii,y+ii, &my_proj);
      if (x[ii]>=x1 && x[ii]<=x2 && y[ii]>=y1 && y[ii]<=y2) {
/*printf("lat= %f lon= %f   x= %f y= %f\n", oneRecord.pts[ii].lat, oneRecord.pts[ii].lon, x[ii], y[ii]); */
	allout = 0;
	inarea[ii] = 1;
	minreclat = min(oneRecord.pts[ii].lat,minreclat);
	minreclon = min(oneRecord.pts[ii].lon,minreclon);
    ...
    if (allin) {
      ...
      dump_xy_to_gnuplot(ofp,x,y,oneRecord.npts,cornertocorner); 
      continue;
    } else if (allout)
      continue;
	...
	wy = 1.0;
      ...
      xy_to_ll(&xx[clipRec.npts],&yy[clipRec.npts],&lat,&lon, &my_proj); 
      clipRec.pts[clipRec.npts].lat = lat;
      clipRec.pts[clipRec.npts++].lon = lon;
      if (inarea[ii]) {
        xx[clipRec.npts] = x[ii];
	...
	dump_xy_to_gnuplot(ofp,xx,yy,clipRec.npts,cornertocorner); 
	clipRec.npts = 0;
      }/*endif*/
    }/*end for ii*/
    if (clipRec.npts>1) {
