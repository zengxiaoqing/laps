...
999     continue
     ...
     1                  j_status)
        ...
        real dbz_anal_3d(NX_L,NY_L,NZ_L)
        real dbz_fcst_3d(NX_L,NY_L,NZ_L)
...
!       integer n_models
!       parameter (n_models=2)
!       character*10 c_model_a(n_models)

!       data c_model_a /'wrf-lin','wrf-wsm6'/        
        ...
        real cont_4d(NX_L,NY_L,NZ_L,ndbz)
...
!       Get fdda_model_source from static file
        ...
        do imodel = 1,n_fdda_models
          ...
          if(c_model(1:3) .ne. 'lga')then
            ...
            hist_dir = verif_dir(1:len_verif)//'radar/hist/'
     1                                       //c_model(1:len_model)
            len_hist = len_verif + 11 + len_model

            cont_dir = verif_dir(1:len_verif)//'radar/cont/'
     1                                       //c_model(1:len_model)
     1                                       //'/'
!           len_cont = len_verif + 11 + len_model
...
!             Read analyzed reflectivity
     ...
     1            ,ext,var_2d,units_2d,comment_2d,dbz_anal_3d,istatus)
              if(istatus .ne. 1)then
...
!             Read forecast reflectivity
     ...
     1                          ,directory,var_2d
     1                          ,units_2d,comment_2d,dbz_fcst_3d
     1                          ,istatus)
              if(istatus .ne. 1)then
...
!             Calculate "and" mask
              ...
              do j = 1,NY_L
                ...
                if(dbz_anal_3d(i,j,k) .ne. r_missing_data .and. 
     1             dbz_anal_3d(i,j,k) .ge. thresh_dbz .and.
     1             dbz_fcst_3d(i,j,k) .ne. r_missing_data .and.
     1             dbz_fcst_3d(i,j,k) .ge. thresh_dbz           )then
...
!             Calculate "or" mask
              ...
              do j = 1,NY_L
                ...
                if((dbz_anal_3d(i,j,k) .ne. r_missing_data .and. 
     1              dbz_anal_3d(i,j,k) .ge. thresh_dbz) .OR.
     1             (dbz_fcst_3d(i,j,k) .ne. r_missing_data .and.
     1              dbz_fcst_3d(i,j,k) .ge. thresh_dbz)          )then       
     ...
     1                        ,a9time_valid
              call radarhist(NX_L,NY_L,NZ_L,dbz_anal_3d,lmask_all_3d
     1                     ,lun_out)       

              write(lun_out,*)
              write(lun_out,*)' Calling radarhist for',ihr_fcst
     1               ,' hr forecast valid at ',a9time_valid
              call radarhist(NX_L,NY_L,NZ_L,dbz_fcst_3d,lmask_all_3d
     1                    ,lun_out)

              write(lun_out,*)
              write(lun_out,*)
     1    ' 3-D AND mask is in place with dbz threshold of ',thresh_dbz       

              write(lun_out,*)
              write(lun_out,*)' Calling radarhist for analysis at '
     1                      ,a9time_valid
              call radarhist(NX_L,NY_L,NZ_L,dbz_anal_3d,lmask_and_3d
     1                      ,lun_out)

              write(lun_out,*)
              write(lun_out,*)' Calling radarhist for',ihr_fcst
     1               ,' hr forecast valid at ',a9time_valid
              call radarhist(NX_L,NY_L,NZ_L,dbz_fcst_3d,lmask_and_3d
     1                    ,lun_out)
...
!             Calculate contingency tables
     ...
     1                         ,rdbz,' dbz'
                call contingency_table(dbz_anal_3d,dbz_fcst_3d
