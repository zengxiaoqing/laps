...
! --------------------
  ...
  INTEGER(KIND=4) :: O,S,ER,UU,VV,WW
! --------------------
  UU=U_CMPNNT
  VV=V_CMPNNT
  WW=W_CMPNNT
! READ IN OBSERVATIONS
  ...
  DO O=1,NALLOBS
    ...
    ELSEIF(S.EQ.NUMSTAT+1)THEN  ! FOR RADAR
      ...
      NOBSTAT(UU)=NOBSTAT(UU)+1
      SCL(VV)=SCL(VV)+OBSVALUE(O)*OBSVALUE(O)
      NOBSTAT(VV)=NOBSTAT(VV)+1
    ENDIF
  ENDDO
  DO S=1,NUMSTAT
  ...
  ENDDO
  IF(NUMDIMS.GE.2)SCL(UU)=DMAX1(SCL(UU),SCL(VV))
...
!*************************************************
  IMPLICIT NONE
! --------------------
  INTEGER(KIND=4) :: I,J,K,T,S,NX,NY,NZ,NT,I1,J1,K1,T1,UU,VV,PP
  REAL(KIND=8)    :: Z1,Z2
! --------------------
  UU=U_CMPNNT
  VV=V_CMPNNT
...
! PENALTY COEFFICENT
  ...
  ENDDO
  PNLT_PU=PNLT0PU*SCL(UU)*SCL(UU)*NALLOBS/NUMSTAT/(2**(GRDLEVL-1)) &
              /(NUMGRID(1)*NUMGRID(2)*NUMGRID(3)*NUMGRID(4))
  PNLT_PV=PNLT0PV*SCL(VV)*SCL(VV)*NALLOBS/NUMSTAT/(2**(GRDLEVL-1)) &
              /(NUMGRID(1)*NUMGRID(2)*NUMGRID(3)*NUMGRID(4))
...
! CALCULATE SCALE FOR PHYSICAL POSITION AND CORIOLIS FREQUENCY
  SCP(XSL)=DABS((XXX(NUMGRID(1),NUMGRID(2))-XXX(1,1))/(NUMGRID(1)-1))
  SCP(YSL)=DABS((YYY(NUMGRID(1),NUMGRID(2))-YYY(1,1))/(NUMGRID(2)-1))
  IF(IFPCDNT.EQ.0)THEN
    ...
    ENDDO
    ENDDO
    SCP(PSL)=DABS((Z2-Z1)/(NUMGRID(3)-1))
  ELSEIF(IFPCDNT.EQ.1)THEN
    SCP(PSL)=DABS((PPP(NUMGRID(3))-PPP(1))/(NUMGRID(3)-1))
  ENDIF
  IF(PNLT0PU.GE.1.0E-10.OR.PNLT0PV.GE.1.0E-10)SCP(CSL)=DABS(COR(1,1))
  DO I=1,NUMGRID(1)
  DO J=1,NUMGRID(2)
    IF(PNLT0PU.GE.1.0E-10.OR.PNLT0PV.GE.1.0E-10)SCP(CSL)=DMAX1(SCP(CSL),DABS(COR(I,J)))
  ENDDO
  ENDDO
  IF(PNLT0PU.GE.1.0E-10.OR.PNLT0PV.GE.1.0E-10)SCP(DSL)=DABS(DEN(1,1,1,1))
  DO T=1,NUMGRID(4)
  DO K=1,NUMGRID(3)
  DO J=1,NUMGRID(2)
  DO I=1,NUMGRID(1)
    IF(PNLT0PU.GE.1.0E-10.OR.PNLT0PV.GE.1.0E-10)SCP(DSL)=DMAX1(SCP(DSL),DABS(DEN(I,J,K,T)))
  ENDDO
  ENDDO
  ENDDO
  ENDDO
