MODULE INPUT_BG_OBS
  ...
  PUBLIC           BKGRNDOBS
  PRIVATE          READNMLST, BKGMEMALC, OBSMEMALC, MEMORYALC, MEMORYRLS, GETBKGRND, ADDBKGRND, GETBKGRND_NEW
...
!  OPEN(NU,FILE=DR(1:N+10),ACTION='READ',STATUS='OLD')
  ...
  DO S=1,NUMSTAT
    READ(NU,*)SL0(S)
  ENDDO
  ALLOCATE(PENAL0X(NUMSTAT+1),STAT=ER)
  IF(ER.NE.0)STOP 'PENAL0X ALLOCATE WRONG'
  ALLOCATE(PENAL0Y(NUMSTAT+1),STAT=ER)
  IF(ER.NE.0)STOP 'PENAL0Y ALLOCATE WRONG'
  ALLOCATE(PENAL0Z(NUMSTAT+1),STAT=ER)
  IF(ER.NE.0)STOP 'PENAL0Z ALLOCATE WRONG'
  ALLOCATE(PENAL0T(NUMSTAT+1),STAT=ER)
  IF(ER.NE.0)STOP 'PENAL0T ALLOCATE WRONG'
  ALLOCATE(PENAL_X(NUMSTAT+1),STAT=ER)
  IF(ER.NE.0)STOP 'PENAL_X ALLOCATE WRONG'
  ALLOCATE(PENAL_Y(NUMSTAT+1),STAT=ER)
  IF(ER.NE.0)STOP 'PENAL_Y ALLOCATE WRONG'
  ALLOCATE(PENAL_Z(NUMSTAT+1),STAT=ER)
  IF(ER.NE.0)STOP 'PENAL_Z ALLOCATE WRONG'
  ALLOCATE(PENAL_T(NUMSTAT+1),STAT=ER)
...
! --------------------
  INTEGER  :: ER
! --------------------
  ALLOCATE(GRDBKGD0(MAXGRID(1),MAXGRID(2),MAXGRID(3),MAXGRID(4),NUMSTAT+1),STAT=ER)
