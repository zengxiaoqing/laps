...
print*,'SPECIFIC: ',minval(GRDANALS(1:numgrid(1),1:numgrid(2),1:numgrid(3),2,5)+ &
  ...
  ! NOV. 2009 BY YUANFU: SKIP THE FOLLOWING UNSCALING AS
  ! WWW SAVES THE FULL OMEGA FIELD NOT VERTICAL VELOCITY ANYMORE
  ! DO T=1,NUMGRID(4)
  ! DO K=1,NUMGRID(3)
  ! DO J=1,NUMGRID(2)
  ! DO I=1,NUMGRID(1)
...
!*************************************************
  IMPLICIT NONE
  ! DEALLOCATE(WWW)	! DELAY THE DEALLOCATION TO OUTPUT ROUTINES
  DEALLOCATE(COR)
  DEALLOCATE(XXX)
  DEALLOCATE(YYY)
  DEALLOCATE(DEG)
...
!  CLOSE(2)
  DO S=1,NUMSTAT
    ...
    DO J=1,MAXGRID(2)
    DO I=1,MAXGRID(1)
      GRDBKGD0(I,J,K,T,S)=GRDANALS(I,J,K,T,S) 	!+GRDBKGD0(I,J,K,T,S) BK0 is added in LAPSOUTPUT routine
    ENDDO
    ENDDO
    ENDDO
    ENDDO
