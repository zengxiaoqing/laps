...
cdis 
cdis 
        subroutine comp_maps_prof(u_bkg,v_bkg,ni,nj,nk
     1  ,grid_laps_u,grid_laps_v,grid_laps_wt,weight_prof,r_missing_data
     1  ,rms)
...
!           if(grid_laps_wt(il,jl,nk/2) .eq. weight_prof)then ! for speed

              do k = 1,nk

                if(grid_laps_wt(il,jl,k) .eq. weight_prof
...
!           endif ! There is a profiler in this column
