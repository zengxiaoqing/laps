...
c     subroutine allocate_bg_arrays(ia,nx,ny,
c    .     nz_ht,nz_sh,nz_uv,nz_ww,
c    .     prbght,prbgsh,prbguv,prbgww,
c    .     htbg,tpbg,uwbg,vwbg,shbg,wwbg,
c    .     htbg_sfc,prbg_sfc,shbg_sfc,tpbg_sfc,
c    .     uwbg_sfc,vwbg_sfc,mslpbg,istatus)
...
c     real, allocatable :: htbg(:,:,:)     !Height (m)
c     real, allocatable :: tpbg(:,:,:)     !Temperature (K)
c     real, allocatable :: shbg(:,:,:)     !Specific humidity (kg/kg)
c     real, allocatable :: uwbg(:,:,:)     !U-wind (m/s)
c     real, allocatable :: vwbg(:,:,:)     !V-wind (m/s)
c     real, allocatable :: wwbg(:,:,:)     !W-wind (pa/s)

c     real, allocatable :: mslpbg(:,:)     !mslp  (mb)
c     real, allocatable :: htbg_sfc(:,:)
c     real, allocatable :: prbg_sfc(:,:)
c     real, allocatable :: shbg_sfc(:,:)
c     real, allocatable :: uwbg_sfc(:,:)
c     real, allocatable :: vwbg_sfc(:,:)
c     real, allocatable :: tpbg_sfc(:,:)

c     real, allocatable :: prbght(:,:,:)     !Pressure (mb) ht and temp
c     real, allocatable :: prbgsh(:,:,:)     !Pressure (mb) q
c     real, allocatable :: prbguv(:,:,:)     !Pressure (mb) u- v-components
c     real, allocatable :: prbgww(:,:,:)     !Pressure (mb) omega

c     if(ia.eq.1)then

c        allocate (htbg(nx,ny,nz_ht))
c        allocate (tpbg(nx,ny,nz_ht))
c        allocate (shbg(nx,ny,nz_sh))
c        allocate (uwbg(nx,ny,nz_uv))
c        allocate (vwbg(nx,ny,nz_uv))
c        allocate (wwbg(nx,ny,nz_ww))
c        allocate (prbght(nx,ny,nz_ht))
c        allocate (prbguv(nx,ny,nz_uv))
c        allocate (prbgsh(nx,ny,nz_sh))
c        allocate (prbgww(nx,ny,nz_ww))

c        allocate (htbg(nx,ny))
c        allocate (prbg(nx,ny))
c        allocate (shbg(nx,ny))
c        allocate (uwbg(nx,ny))
c        allocate (vwbg(nx,ny))
c        allocate (tpbg(nx,ny))
c        allocate (mslpbg(nx,ny))
...
c        deallocate (htbg_sfc)
c        deallocate (prbg_sfc)
c        deallocate (shbg_sfc)
c deallocate (uwbg_sfc)
c deallocate (vwbg_sfc)
c        deallocate (tpbg_sfc)
c        deallocate (mslpbg)
