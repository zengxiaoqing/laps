...
c
c       Minpack2 Library ... timer_lbfgs, epmeps.
c
...
c
c     d is a real working array of dimension n.
c       d will be used to store the Cauchy direction P(x-tg)-x.
c
c     xcp is a real working array of dimension n.
...
c======================= The end of errchk =============================
      ...
      real rr,sg(m),yg(m),sgo(m),ygo(m),sy(m,m),yy(m,m)
...
c       On exit sy is unchanged.
c
c     yy is a real array of dimension m x m.
...
c======================== The end of hmv ===============================
     ...
     +                  info,task,boxed,cnstnd,csave,isave,dsave)
      ...
      real f,fold,gd,gdold,stp,dnorm,dtd,xstep,stpmx,
...
c======================= The end of print2 =============================
     ...
     +        nact,nint,iback,k,nbd(n)
      real f,sbgnrm,time,stp,xstep,cachyt,sbtime,lnscht,
     +                 x(n),l(n),u(n)
...
c       On exit ind is unchanged.
c
c     v is a real array of dimension nsub.
c       On entry v specifies vector v.
c       On exit v is unchanged.
c
c     ws, wy, sy, and wt are real arrays;
...
c       On exit they are unchanged.
c
c     v is a real working array of dimension nsub.
c       On entry v is the vector v.
c       On exit v is unchanged.
c
c     p is a real working array of dimension nsub.
c       On entry p is unspecified.
c       On exit p is the product Z^(T)BZ*v.
c
c     wa is a real working array of dimension 2col.
c
c     info is an integer variable.
c       On entry info is 0.
c       On exit info = 0       for normal return,
...
c
c     r is a real working array of dimension n.
c       On entry r is minus the reduced gradient of the model
c       at xcp.
c
c     p is a real working array of dimension nsub.
c
c     q is a real working array of dimension nsub.
c
c     m is an integer variable.
c       On entry m is the maximum number of variable metric 
c          corrections allowed in the limited memory matrix.
...
c     ************
      ...
      real ddot,rho1,rho2,beta,alpha1,alpha2,epsil,
...
c     Set tolerance.
      ...
      epsil = (min(1.0e-2,sqrt(rho2)))*rho2
      m2 = 2*m
...
c       On exit x is unchanged.
c
c     xcp is a real array of dimension n.
...
c     d is a real working array of dimension n.
c
c     tlmda is a real working array of dimension n. 
c
c     ws, wy, sy, and yy are real arrays;
c     theta is a real variable;
c     col is an integer variable;
c     head is an integer variable.
c       On entry they store the information defining the
...
c       On exit they are unchanged.
c
c     sg is a real array of dimension m.
c       On entry sg specifies S'g.
c       On exit sg is unchanged.
c
c     yg is a real array of dimension m.
...
c
c     wv and  wv2 are real working arrays of dimension 2m.
c
c     wn is a real array of dimension 2m x 2m.
...
c     ************
      ...
      real t1,t2,sum1,sum2,alpha,theta2,a1,a2,dk
...
c                 1 if some bound is encountered.
c
c     wv are real working arrays of dimension 2m.
c
c     wn is a real array of dimension 2m x 2m.
...
c====================== The end of subsm ===============================
      ...
      real g(n),ws(n,m),wy(n,m),sg(m),yg(m),sgo(m),ygo(m)
...
c       On exit n is unchanged.
c
c     t is an real array of dimension n.
...
c     to subroutine timer_lbfgs to determine the initial and final time.
c
c     The subroutine statement is
c
c       subroutine timer_lbfgs(ttime)
c
c     where
c
