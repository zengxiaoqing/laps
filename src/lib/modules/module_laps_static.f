...
MODULE laps_static
  ...
  INTEGER                     :: nk             ! Z-dimension of laps domain
...
CONTAINS
  ...
  SUBROUTINE get_horiz_grid_spec(laps_data_root)
    ...
    IF(istatus.ne.1)then
       print*,'Error getting nk - get_horiz_grid_spec'
       stop
    ENDIF
    ...
    IF(.not. allocated(lats))ALLOCATE(lats(x,y))
    IF(.not. allocated(lons))ALLOCATE(lons(x,y))
    ...
    IF(.not. allocated(topo))ALLOCATE(topo(x,y))
