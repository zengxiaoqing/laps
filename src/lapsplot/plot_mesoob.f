...
!     call supcon(alat,alon,x1,y1)
      ...
      v = rj
        ...
        if(t.gt.-75. .and. t.lt.140.) then
           ...
           call pwrity(u-du_t,v+dv,t1,3,-1,0,-1)
        endif
 100    format(i3)
c
 20     if(td.gt.-75. .and. td.lt.100.) then
           write(td1,100,err=30) nint(td)
           call pwrity(u-du_t,v-dv,td1,3,-1,0,-1)
        endif
c
 30     if(p .gt. 0.) then
           ...
           call pwrity(u+du_p,v+dv,p1,3,-1,0,0)
        endif
        ...
        if(iflag .eq. 1)then ! Plot Gusts (FSL WWW)
           if(gust .gt. 40)then
               ...
               dg = 3.0 * du
!              call pwrity(u,v+dg,p1,3,-1,0,0)          ! On Top
               call pwrity(u+du_p,v-dv,p1,3,-1,0,0)     ! Lower Right
 102           format('G',i2)
