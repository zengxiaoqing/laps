...
!       Note that this routine assumes a polar stereographic projection
...
d       WRITE(6,18)
d18     FORMAT('        CALCULATING FLUX CONVERGENCE FIELD')
...
C       THIS VALUE REPRESENTS  (M*RHO(EARTH)*(1.+SIN(PHI0))/MPIN)**2
        ARGM=1.+SIN(PHI0*RPD)
        CONST=(M*6371000.*ARGM/.0254)**2
        DO 100 I=1,NI
        DO 100 J=1,NJ
            sinphi=sind(lat(i,j))
            SIGMA(I,J)=ARGM/(1.+SINPHI)
!           sigma(i,j)=1.0
C
