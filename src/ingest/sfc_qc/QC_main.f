...
c
       call QC_main_sub(maxstns_cmn,laps_cycle_time_cmn,badflag)
c     
       end
c
c
       subroutine QC_main_sub(m, laps_cycle_time, badflag)
c
c*********************************************************************
c
...
c
	integer     i4time, i4prev1, i4prev2
	character*9 filename, fname1, fname2
        Character   atime*24,stn(m)*5,wx(m)*25
        character   stna(m)*5
        character   stnb(m)*5
c
c
...
c      is calculated in the LS2 file, and is passed in by
c      the read_surface_LS2 routine. ***
c
...
c
        ...
        enddo !i
c
c.....  Check for a 'monster' file.  If there, read it.
...
c              
              iiiii=ran(iiiii)*2000000.-1000000.
...
c
     ...
     &  wmoid,stations,provider,wx,reptype,autostntype,latb,lonb,elevb,
     &  tb,tdb,rh,ddb,ffb,ddg,ffg,altb,pstnb,pmslb,delpch,delp,vis,solar
...
c
        if(jstatus .ne. 1) then
           print *,' No LSO data for (Hr-2). No Kalman QC for ',filename
           stop
        else
...
c
     ...
     &  wmoid,stations,provider,wx,reptype,autostntype,lata,lona,eleva,
     &  ta,tda,rh,dda,ffa,ddg,ffg,alta,pstna,pmsla,delpch,delp,vis,solar
     &  ,sfct,sfcm,pcp1,pcp3,pcp6,pcp24,snow,kkk_s,max24t,min24t,t_ea,
     &  td_ea,rh_ea,dd_ea,ff_ea,alt_ea,p_ea,vis_ea,solar_ea,sfct_ea,
     &  sfcm_ea,pcp_ea,snow_ea,store_cldamt,store_cldht,
     &  m,jstatus)
c
        if(jstatus .ne. 1) then
           print *,' No LSO data for (Hr-1). No Kalman QC for ',filename
           stop
        else
...
c
     ...
     &       stna,tb,tdb,ddb,ffb,latb,lonb,elevb,pstnb,pmslb,altb,stnb,
...
c
      ELSE
*************READ(wherever)vit,vitd,vitu,vitv,vitpm,vital
*************READ(wherever)wit,witd,witu,witv,witpm,wital
*************READ(wherever)pt,ptd,pu,pv,ppm,pal
*************READ(wherever)monster
*************READ(wherever)ta,tda,ua,va,pmsla,alta
      ENDIF
c
c start of routine
c
...
c
c     use series to estimate next ob/or use taylor series
c
 334       continue
...
*****MODEL IS UNDER REVISION - ARGUEMENTS WILL CHANGE**********
     ...
     1          ,maxstaa,m,timer,cycle,14.,4.,24.,6.)
c
...
c  kalman....
           call kalman(dua,F,yua,pu,it, w,vv,xu,xtu,maxstaa,m,
     &          atime)       
           write(8,*) 'Kalman for V-WIND'
           call model(F,dva,thetaa,ua,va,thetaa,mwt,lata,lona,eleva
     1          ,maxstaa,m,timer,cycle,0.,0.,24.,6.)
c
c  create the v and w matrices using the error history
c
...
c  kalman....
c
           call kalman(dva,F,yva,pv,it, w,vv,xv,xtv,maxstaa,m,
     &          atime)
           write(8,*) 'Kalman for PMSL'
           call model(F,dpma,thetaa,ua,va,thetaa,mwt,lata,lona,eleva
     1          ,maxstaa,m,timer,cycle,0.,3.,16.,16.)
c
c  create the v and w matrices using the error history
c
...
c  kalman....
           call kalman(dpma,F,YpmslA,ppm,it, w,vv,xpm,xtpm,maxstaa,m,
     &          atime)
           write(8,*) 'Kalman for ALSTG'
           call model(F,dalta,thetaa,ua,va,thetaa,mwt,lata,lona,eleva
     1          ,maxstaa,m,timer,cycle,0.,.1,24.,16.)
c
c  create the v and w matrices using the error history
c
...
c  kalman....
           call kalman(dalta,F,yalta,pal,it, w,vv,xal,xtal,maxstaa,m,
     &          atime)
c
...
c
           if(jstatus .ne. 1) then
              ...
              print *,' Found LSO data (current) at ', atime
           endif
c
           maxsta=n_obs_b
           do k=1,maxsta
 ...
 1066      format(1x,a24)
     ...
     &          stn ,ta,tda,dda,ffa,lata,lona,eleva,pstna,pmsla,alta,
     &          stna,maxsta,maxstaa,m,badflag)
...
c
 ...
 3006      format(1x,i3)
           do i=1,maxstaa
c
c.....  Write out the NetCDF QC output file.
c
********write to temploc.YYYYJJJHHMM
 1            write(14,3005) stn(i),lat(i),lon(i),elev(i),
     &             qcstat(i),t(i),tb(i),ta(i),tc(i),te(i),
     &             qcstatd(i),td(i),tdb(i),tda(i),tdc(i),tde(i),
     &             qcstauv(i),u(i),ub(i),ua(i),uc(i),ue(i),
     &             qcstauv(i),v(i),vb(i),va(i),vc(i),ve(i),
     &           qcstapm(i),pmsl(i),pmslb(i),pmsla(i),pmslc(i),pmsle(i),
     &             qcstal(i),alt(i),altb(i),alta(i),altc(i),alte(i)
 3005         format(1x,a5,2x,2f7.2,f7.0,3x,6(i7,5f6.1))
           enddo !i
c
...
c
*************WRITE(wherever)vit,vitd,vitu,vitv,vitpm,vital
*************WRITE(wherever)wit,witd,witu,witv,witpm,wital
*************WRITE(wherever)pt,ptd,pu,pv,ppm,pal
*************WRITE(wherever)monster
*************WRITE(wherever)ta,tda,ua,va,pmsla,alta
c
        return
        end
