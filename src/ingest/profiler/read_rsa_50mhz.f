      ...
      subroutine read_rsa_50mhz(i4time_sys,i4_prof_window
     1                                    ,NX_L,NY_L
     1                                    ,ext
     1                                    ,filename,istatus)
...
C
      ...
      call read_50mhz(nf_fid, level, maxStaticIds, nInventoryBins,
     +     recNum,ext,
!.............................................................................
...
C
C
      subroutine read_50mhz(nf_fid, level, maxStaticIds, nInventoryBins,       
     +     recNum,
!.............................................................................
...
!............................................................................
      ...
      call read_50mhz_netcdf(nf_fid, level, maxStaticIds, 
     +     nInventoryBins,       
     ...
     1            topo_a,1.0,rnorth,south,east,west,istatus)
      ...
      do i_sta = 1,nStaticIds
        ...
        do itimes = i_pr_l,i_pr_h
          ...
          write(6,*)staticIds(i_sta),' pressure... '
...
!             Convert u_std, v_std to rms
              ...
              if(abs(observationTime(i_pr))      .lt. 3d9)then
                  call c_time2fname(nint(observationTime(i_pr))
     1                                    ,a9_timeObs)
              else
     ...
     1                       ,observationTime(i_pr)
              ...
              if(i4_resid .gt. i4_prof_window)then ! outside time window
                  write(6,*)' time - reject '
     1             ,a9_timeObs,i4_resid,i4_prof_window
                  goto 900        
              endif
...
C
              ...
              if(ext(1:3) .eq. 'pro')then
                  ...
                  do i = 1,level
                      if(windDir(i,i_pr) .ge. 0.   .and.
     1                   windDir(i,i_pr) .le. 360.       )then ! Good QC
                          n_good_levels = n_good_levels + 1
                          ht_out(n_good_levels) = levels(i,i_pr)
                          di_out(n_good_levels) = windDir(i,i_pr)
                          sp_out(n_good_levels) = windSpeed(i,i_pr)
                      endif
                  enddo ! i
...
C
      subroutine read_50mhz_netcdf(nf_fid, level, maxStaticIds, 
