...
c
      ...
      real*4    u(nx_l+2,ny_l+2)
      real*4    v(nx_l+2,ny_l+2)
      ...
      character*255 path
      character*200 path_to_raw_satellite
      character*200 cname
...
c
      ...
      logical     lfirst(maxtype,maxsat)
      data lfirst /.false.,.false.,.false.,
     &             .false.,.false.,.false./
      save

      jstatus = -1
c
c nx and ny: an extra row and column "laps domain" used
...
c
c The following routine reads the attributes for the satellite
c data in /data/fxa/SBN/. This allows automatic mapping arrays
c to the LAPS domain.
c
c     if(cdtype.eq.'wfo')then
c        call update_wfo_sat_parms(path_to_raw_wfo_sat(kchl,jtype,
c    &isat),rla1,rlo1,rsat_sub_lat,rsat_sub_lon,dx,dy,sublat,sublon
c     )
...
c
      call get_sat_boundary(nx,ny,ny3,nx3,ri,rj,
     &linestart,lineend,elemstart,elemend,
     &rls,rle,res,ree,istatus)
      if(istatus.ne.1)then
         write(6,*)'Fatal Error in get_sat_boundary - gen_cdf_lut'
