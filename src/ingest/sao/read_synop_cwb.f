...
20    format ( 2x, 2f3.0, i1, f2.0, a2, 3x, f5.1, i1, f4.1, i1, a2, 2x,
     ~         f2.0 )
30    format ( f3.1, i1, x, i2, f3.1, i1, 3(x, f4.1), 5x, 2f3.0 )
...
99    do j= 1,n
         ...
         if ( windGust(j) .eq. -99. )  then
               windGust(j)= badflag
            else
               windGust(j)= windGust(j) *0.518          ! knot -> meter/second
         endif
...
999   do j= 1,n
     ...
     ~               temperature(j), temperatureQua(j),
     ~               skyCover(1,j), skyLayerBase(1,j)
         write (6,*) tempDewDiff(j), dewpointQua(j),
     ~               pressChangeChar(j), pressChange3Hour(j),
     ~               pressChange3HourQua(j), precip3Hour(j),
     ~               maxTemp24Hour(j), minTemp24Hour(j), windGust(j) 
         write (6,*) skyCover(2,j), skyLayerBase(2,j),
     ~               precip24Hour(j), timeobs(j)
      enddo
